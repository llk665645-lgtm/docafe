...
<template>
  <form :class="cn('flex flex-col gap-6', props.class)" @submit.prevent="handleSubmit">
    <FieldGroup>
      ...
      <div v-if="error" class="bg-destructive/15 text-destructive text-sm p-3 rounded-md mb-4 text-center">
        {{ error }}
      </div>
      ...
      <Field>
        <Button variant="outline" type="button" @click="() => navigateTo('http://127.0.0.1:8000/auth/github', { external: true })">
